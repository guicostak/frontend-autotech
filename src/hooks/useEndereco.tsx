...
import { getUserById, buscarEnderecoPorCep, atualizarEndereco, preencherEndereco } from '@/services/userService'; 
...
export default function useEndereco() {
    ...
    const userId: number = id ? parseInt(id, 10) : -1;  
    const [isAddressExisting, setIsAddressExisting] = useState(false);
    const [perfilIncompleto, setPerfilIncompleto] = useState<boolean>(false);
    ...
    setPerfilIncompleto(!isComplete);
    };    
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (validateAllFields() || userId != null) {
            try {
                ...
                await preencherEndereco(userId, cleanedFormData);
                ...
                await atualizarEndereco(userId, cleanedFormData);
